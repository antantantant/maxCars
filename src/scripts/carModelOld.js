function carModel(Input, order, step, wheel_n, wheel_step){
    ...
    temp = Math.min((1.0 + (roof_end_pt_x - roof_start_pt[0])* Input[6])*roof_vec[2]/roof_vec[0]+roof_start_pt[2], roof_end_pt_z+0.2);
    ...
    var c1_t = 0.8 + 0.2* Input[9]; //scale-copy the profile on xz plane to the side curve
    ...
    var hood_u_pt_z = -0.2* Input[10]; //move side curve of component1 on the z axis
    var hood_u_pt = [hood_end_pt[0] + hood_u_pt_x, 1.8+0.2* Input[11], hood_end_pt[2] + hood_u_pt_z];
...
//    bw_pt[1] -= 0.2*Input[32];
    ...
    var box_w = 2.1 + 0.6 * Input[12];
    var nose_mid_pt = [-2.0 - origin[0], 0 - origin[1], 0 - origin[2]];
    var hood_start_pt = [nose_mid_pt[0]  + 0.5* Input[13], 0.0, hood_end_pt[2] - 1.0* Input[14]];
    var temp = normalize(dvec(hood_start_pt, hood_end_pt))[2] * (-0.4 + 1.5 * Input[15]);
    var hood_vec = [-Math.sqrt(1.0-temp*temp), 0.0, temp];
    var hood_n = (hood_start_pt[0] - hood_end_pt[0])*0.1/ hood_vec[0] +  (hood_start_pt[0] - hood_end_pt[0]) * 0.9 /  hood_vec[0] * Input[16];
    ...
    var nose_u_pt = [hood_start_pt[0] + 0.1 + 0.4* Input[17], hood_u_pt[1] - 0.3 - 0.6* Input[18], hood_start_pt[2]];
    temp = normalize(dvec(nose_u_pt, hood_u_pt));
    var hood_v_vec = [temp[0], temp[1], temp[2] * (0.0 + 0.8 * Input[19])];
    hood_v_vec = normalize(hood_v_vec);
    var hood_v_n = (nose_u_pt[0] - hood_u_pt[0])*0.8/ hood_v_vec[0] +  (nose_u_pt[0] - hood_u_pt[0])*0.2/ hood_v_vec[0]* Input[20];
    ...
    var head_end_pt = [head_u_hash_pt[0] + 0.1 + 0.2* Input[22], nose_u_pt[1] - (nose_u_pt[1] - head_u_hash_pt[1])/2*Input[23], head_u_hash_pt[2]];
...
//    
    ...
    var well_pt = intersect(well_u_mid_pt, dvec(well_u_mid_pt, hood_u_pt), [0, head_u1_pt[1], 0], [0.000001, 1, 0.000001]); //simplified
    well_pt[2] = Math.max(well_pt[2], 0.5);
    well_u_mid_pt[1] = Math.min(well_u_mid_pt[1], well_pt[1]);

    var head_center_vec = dvec(nose_v_hash_pt, nose_center); //simplied..**
    temp_vec = intersect(nose_v_hash_pt, [head_center_vec[0], head_center_vec[1], 0], pvec(nose_v_hash_pt, [0, temp*0.5, 0]), well_v_vec);
    if(temp_vec[0]<intersect(nose_v_hash_pt, dvec(nose_v_hash_pt, mid_pt1), pvec(nose_v_hash_pt, [0, temp*0.5, 0]), well_v_vec)[0]
    	&& temp_vec[0]>nose_mid_pt[0]){
    	var head_center = temp_vec;
    }
    else{
    	var head_center = intersect(nose_v_hash_pt, dvec(nose_v_hash_pt, mid_pt1), pvec(nose_v_hash_pt, [0, temp*0.5, 0]), well_v_vec);
    }
    ...
    var head_u2_mid_pt = intersect(nose_u_pt, dvec(nose_u_pt, mid_pt1), pvec(nose_u_pt,[0, temp*Input[25], 0]), well_v_vec);
	head_u2_mid_pt[1]+=(Input[26]-0.1)*0.2;
	head_u2_mid_pt[2]+=(Input[27]-0.5)*0.5;
	head_u2_mid_pt[0] = Math.max(head_u2_mid_pt[0], head_center[0]+0.1);
	
    var well_vvv_vec = normalize(dvec(head_u2_mid_pt, head_center));
    temp = Math.min((well_u_mid_pt[0]-head_u2_mid_pt[0])/well_vvv_vec[0]/2, Math.max((well_u_mid_pt[2]-head_u2_mid_pt[2]+0.5)/well_vvv_vec[2], 0.01));
...
//    var front_well_pt1 = [fwheel_x + r_wheel*well_scale, head_u1_pt[1]*0.2+box_w*0.8, 0.0];
    ...
    var front_well_pt2 = [well_pt[0] - well_v_pt[0] + front_well_pt1[0], front_well_pt1[1] - 0.1* Input[28], front_well_pt1[2]];
...
//    temp_vec = normalize([hood_u_pt[0]-hood_v_pt[0], hood_u_pt[1]-hood_v_pt[1], hood_u_pt[2] - hood_end_pt[2]]);
    ...
    if (temp_vec[2] > -1*Math.pow(10,-6)){
        ...
        var belt_n = temp2* 0.5 + (temp1 - temp2)* 0.5*Input[29];}
    else {
        temp2 = (fw_u_pt[0] - hood_u_pt[0])/temp_vec[0];
        temp1 = Math.min((bw_pt[0] - hood_u_pt[0])/temp_vec[0], (rear_well_pt2[2] - hood_u_pt[2])/temp_vec[2]);
        var belt_n = Math.min(temp1, temp2)* 0.5*(Input[29]+0.01);}
    
    var belt_line_vec = [temp_vec[0], (box_w-hood_u_pt[1])/belt_n* Input[30], temp_vec[2]];
...
//    var rear_well_center2 = intersect(rear_well_v_mid_pt, [(front_well_u2_pt[0] - well_pt[0]) -temp*5, -0.2, 1.0], rear_well_u2_mid_pt, belt_u_vec2);
    ...
    var hip_vec = normalize(pvec(dvec(bw_end_pt, bw_ctrl_pt),[0, 0, 0.5+1*Input[24]]));
//    if (hip_vec[2] < 0){
...
//    }
    hip_n = (hip_mid_pt[0]-bw_end_pt[0])/hip_vec[0];
    ...
    var rear_n = (rear_pt[1] - hip_mid_pt[1])*0.6;
    ...
    var hip_v_mid_pt = intersect(bw_pt, dvec(bw_pt, bw_v_mid_pt), hip_center, [hip_u_vec[0], hip_u_vec[1]+0.000001, hip_u_vec[2]]);
...
//    var rw_mid3 = [rear_well_pt2[0] - dvec(rw_mid1, rear_pt)[0], rear_well_pt2[1] + dvec(rw_mid1, rear_pt)[1], rear_well_pt2[2] + dvec(rw_mid1, rear_pt)[2]];
    ...
    temp = (rw3[0]-rear_well_pt2[0])/(rear_well_pt2[0]-rear_well_v2_mid_pt[0]);
    var rw_mid3 = [rear_well_pt2[0]+ (rear_well_pt2[0]-rear_well_v2_mid_pt[0])*temp*0.5,
                   rear_well_pt2[1] + (rear_well_pt2[1]-rear_well_v2_mid_pt[1])*temp*0.5,
                   rear_well_pt2[2] + (rear_well_pt2[2]-rear_well_v2_mid_pt[2])*temp*0.5];
...
//    var rmid_pt4 = [rw_mid3[0] + (rear_well_u2_mid_pt[0] - rear_well_pt2[0])*k, rw_mid3[1], rw_mid3[2] + (rear_well_u2_mid_pt[2] - rear_well_pt2[2])*k];
    temp = (rmid_pt2[0]-rear_well_u2_mid_pt[0])/(rear_well_u2_mid_pt[0]-rear_well_center2[0]);
    var rmid_pt4 = [rear_well_u2_mid_pt[0] + (rear_well_u2_mid_pt[0]-rear_well_center2[0])*temp*0.5,
                    rear_well_u2_mid_pt[1] + (rear_well_u2_mid_pt[1]-rear_well_center2[1])*temp*0.5,
                    rear_well_u2_mid_pt[2] + (rear_well_u2_mid_pt[2]-rear_well_center2[2])*temp*0.5];
...
//    }
	...
	// rear wheel
	var rearwheel_pts = [];
    wheel_center = [rwheel_x, box_w-1.3+wheel_scale, origin[2]];
    rearwheel_pts = rearwheel_pts.concat(wheel_center);
	...
	for (j = 0; j<t_n+1; j++){
		...
		temp = temp.concat([wheel_center[0]+t_r1*Math.cos(ang_now),
		                    wheel_center[1]-t_r1*Math.tan(t_a1),
		                    wheel_center[2]+t_r1*Math.sin(ang_now)]);
		temp = temp.concat([wheel_center[0]+t_r2*Math.cos(ang_now)*(wheel_scale-0.1),
		                    wheel_center[1]-t_r2*Math.tan(t_a2),
		                    wheel_center[2]+t_r2*Math.sin(ang_now)*(wheel_scale-0.1)]);
		temp = temp.concat([wheel_center[0]+t_r3*Math.cos(ang_now)*(wheel_scale-0.1),
		                    wheel_center[1]-t_r3*Math.tan(t_a3),
		                    wheel_center[2]+t_r3*Math.sin(ang_now)*(wheel_scale-0.1)]);
		temp = temp.concat([wheel_center[0]+t_r4*Math.cos(ang_now)*(wheel_scale-0.1),
		                    wheel_center[1]-t_r4*Math.tan(t_a4),
		                    wheel_center[2]+t_r4*Math.sin(ang_now)*(wheel_scale-0.1)]);
		temp = temp.concat([wheel_center[0]+t_r5*Math.cos(ang_now),
		                    wheel_center[1]-t_r5*Math.tan(t_a5),
		                    wheel_center[2]+t_r5*Math.sin(ang_now)]);
	}
	// rotate temp
	rearwheel_pts = rearwheel_pts.concat(temp);
	for (var j = 0; j<wheel_n-1; j++){
		temp = rotate_y(temp, ang_a, wheel_center);
		rearwheel_pts = rearwheel_pts.concat(temp);
	}
	
	wheel_pts = wheel_pts.concat(rearwheel_pts);
    ...
    //draw wheels
    vertexPositionData = vertexPositionData.concat(wheel_pts);
