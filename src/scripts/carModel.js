...
//    bw_pt[1] -= 0.2*Input[32];
    ...
    var hood_start_pt = [nose_mid_pt[0]  + 0.5* Input[9], 0.0, hood_end_pt[2] - 1.0* Input[10]];
    var temp = normalize(dvec(hood_start_pt, hood_end_pt))[2] * (-0.4 + 1.5 * Input[11]);
    var hood_vec = [-Math.sqrt(1.0-temp*temp), 0.0, temp];
    var hood_n = (hood_start_pt[0] - hood_end_pt[0])*0.1/ hood_vec[0] +  (hood_start_pt[0] - hood_end_pt[0]) * 0.9 /  hood_vec[0] * Input[12];
    ...
    var nose_u_pt = [hood_start_pt[0] + 0.1 + 0.4* Input[13], hood_u_pt[1] - 0.3 - 0.6* Input[14], hood_start_pt[2]];
    temp = normalize(dvec(nose_u_pt, hood_u_pt));
    var hood_v_vec = [temp[0], temp[1], temp[2] * (0.0 + 0.8 * Input[15])];
    ...
    var head_end_pt = [head_u_hash_pt[0] + 0.1 + 0.2* Input[16], nose_u_pt[1] - (nose_u_pt[1] - head_u_hash_pt[1])/2*Input[17], head_u_hash_pt[2]];
...
//    
	...
	head_u2_mid_pt[1]+=(Input[18]-0.1)*0.2;
	head_u2_mid_pt[2]+=(Input[19]-0.5)*0.5;
