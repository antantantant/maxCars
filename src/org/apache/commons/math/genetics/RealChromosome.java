...
public class RealChromosome extends AbstractListChromosome<Double> {
		...
		public AbstractListChromosome<Double> newFixedLengthChromosome(
				...
				double[][] X, double[][] sv, double[] w, double lambda, double[] mean, double[] std, int[][] IND) {
			RealChromosome c = new RealChromosome(
					representation, calculateFitness(list2prim(representation),
							sv, w, lambda, mean, std, IND));
			return c;
		}
		...
		private double calculateFitness(double[] x, double[][] sv, double[] w, double lambda, double[] mean, double[] std, int[][] IND) {
			double f = decisionf(x, sv, w, lambda, mean, std, IND);
			double d = distance(x,sv,lambda,mean,std);
			double fitness = merit(1.0,1.0,f,d);
			return fitness;
		}
		
		private double decisionf(double[] x, double[][] X, double[] w, double lambda, double[] mean, double[] std, int[][] IND){
			double f = 0;
			int l = w.length;
		...
		private double distance(double[] x, double[][] X, double lambda, double[] mean, double[] std){
			double d = 1e6;
			double[] feature = calOneFeature(x);
			double l = X.length;
			for (int i = 0; i<l; i++){
			...
			for (int i = 0; i<l;i++){
				...
				tempd = kernel(samplefeature,feature,lambda);
				if (tempd<d){d=tempd;}
			}
			return d;
		}
...
//		private double distance(double[] x, double[][] allX, double[][] R) {
//			double lambda = 10;
//			Matrix r = new Matrix(allX.length,1);
//			Matrix ones = new Matrix(allX.length,1);
//			for (int i = 0; i < allX.length; i++) {
//				r.set(i,0,Math.exp(-lambda*defdot(allX[i],x)));
//				ones.set(i,0,1.0);
//			}
//			Matrix Rmat = new Matrix(R);
//			Matrix rt = r.transpose();
//			Matrix onest = ones.transpose();
//			Matrix Rr = Rmat.solve(r); 
//			Matrix Rone = Rmat.solve(ones);
//			double s = 1.0 - rt.times(Rr).get(0,0) + Math.pow((1.0 - onest.times(Rr).get(0,0)), 2)/(onest.times(Rone)).get(0,0);
//			return s;
//		}
